...
public class Account {
    ...
    public boolean creditFunds(Transaction transaction) {
        ...
        if (!transaction.getPlayerId().equals(this.playerId)) {
            return false; // В случае несовпадения playerId, возвращаем false
        }

        //Проверяем, что идентификатор транзакции уникален
        if (checkContain(transaction.getTransactionId())) {
            return false; // В случае совпадения transactionID, возвращаем false
        }

        double amount = transaction.getAmount();
        // Проверяем, что сумма транзакции положительная
        if (amount <= 0) {
            return false; // В случае неположительной суммы, возвращаем false
        }
